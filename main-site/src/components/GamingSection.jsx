        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {gamingArticles.map((article, index) => (
            <div key={index} className="group cursor-pointer">
              {/* Thumbnail Container */}
              <div className="relative aspect-16/10 rounded-2xl overflow-hidden border border-white/10 bg-[#121212] mb-4">
                <img 
                  src={article.image} 
                  alt={article.title}
              ...
              {/* Text Content */}
              <div>
                <div className="flex items-center gap-2 mb-2 text-white/30 text-[9px] font-black uppercase tracking-widest">
                  <Gamepad2 className="w-3 h-3" />
                  {article.readTime}
                </div>
                <h3 className="text-white font-bold text-lg leading-tight group-hover:text-accent transition-colors line-clamp-2">
                  {article.title}
                </h3>
              </div>
            </div>
